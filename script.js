...
fetch('https://api.github.com/repos/tuckerit/lykkestrup.dk/contents/images')
    ...
    .then(data => {
        ...
        if (images.length > 0) {
            ...
            if (navigator.userAgent.toLowerCase().includes('mobile') || navigator.userAgent.toLowerCase().includes('tablet')) {
                // User is on a mobile or tablet device
                imageViewer.addEventListener('touchstart', (event) => {
                    clearInterval(intervalId); // Stop den automatiske skift, når der trykkes
